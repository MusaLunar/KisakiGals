...
/// 标签。页面自己的东西（本地搜索词、滚动位置、入库中的条目）不放这里
/// ——换筛选条件才重新拉榜单，输入框只过滤已加载的条目。
library;
...
class DiscoverFilter {
  ...
  /// 清空筛选条件（保留来源与排序）。
  DiscoverFilter cleared() =>
      DiscoverFilter(source: source, sort: sort, onlySfw: true);
  ...
  int get hashCode => Object.hash(source, sort, minRating, yearFrom, yearTo,
      onlySfw, Object.hashAllUnordered(tagIds));
